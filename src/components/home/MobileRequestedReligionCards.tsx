...
interface MobileRequestedReligionCardsProps {
  onReligionClick: (religion: string) => void;
  getClickCount: (religion: string) => string | number;
}

export function MobileRequestedReligionCards({
  onReligionClick,
  getClickCount,
}: MobileRequestedReligionCardsProps) {
  ...
  const onTouchEnd = () => {
    ...
    if (isLeftSwipe && currentIndex < REQUESTED_RELIGIONS.length - 1) {
  ...
  return (
    <div className="relative w-full max-w-xs h-80 overflow-hidden">
      ...
      >
        {REQUESTED_RELIGIONS.map((religion, index) => (
          <div
            key={religion.name}
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
        {REQUESTED_RELIGIONS.map((_, index) => (
          <button
            key={index}
      ...
      {currentIndex < REQUESTED_RELIGIONS.length - 1 && (
